...
const speechLanguages = {
  ...
  മലയാളം: 'ml-IN', मराठी: 'mr-IN', বাংলা: 'bn-IN', ગુજરાતી: 'gu-IN', ਪੰਜਾਬੀ: 'pa-IN'
};
...
function App() {
  ...
  return (
    <div className="app">
      ...
      <div className="main">
        <Header onMenu={() => setMenuOpen(true)} onSearch={(q) => { setQuery(q); set('recentSearches', [q, ...get('recentSearches', [])].filter(Boolean).slice(0, 10)); setPage('Search'); }} />
        <main>{content}</main>
      </div>
      {modal && (
  ...
  return (
    <>
      ...
      <div className="panel">
        ...
        <div className="table-wrap"><table><thead><tr><th>Standard</th><th>Title</th><th>Domain</th><th>Status</th><th>Year</th><th>Match</th></tr></thead><tbody>
          {searchResults.map((s) => <tr key={s.id}><td><b>{s.standardNumber}</b></td><td>{s.title}</td><td>{s.domain}</td><td><span className="badge green">{s.status}</span></td><td>{s.year}</td><td><strong>{analysis?.recommendations?.find((x) => x.id === s.id)?.score || '—'}%</strong></td></tr>)}
        </tbody></table></div>
      </div>
    </>
  );
